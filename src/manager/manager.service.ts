...
export class managerService {
  constructor(
    @InjectModel('manager') private readonly userModel: Model<userDocument>,
    private readonly userService: UserService,
  ) {}

  async createManager(baseUserDto: BaseUserDto): Promise<userDocument> {
    const newManager = this.userService.createOne({
      ...baseUserDto,
      role: role.MANAGER,
    });
    return newManager;
  ...
  async getAllManagers(): Promise<Counted<userDocument>> {
    const managerDetails = await this.userService.getAllManagers();
    const managerCount = managerDetails.length;
    return {
      items: managerDetails,
      count: managerCount,
    };
  }
  async updateManager(
    ...
    const existingManager = this.userService.updateUser(
      managerId,
      updateUserDto,
    );
